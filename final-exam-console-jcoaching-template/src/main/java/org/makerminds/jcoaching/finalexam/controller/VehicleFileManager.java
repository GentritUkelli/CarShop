...
public class VehicleFileManager {
	...
	public List<String> importVehiclesFromFile() {
		...
		try {
			...
			throw new RuntimeException("An erorr occured while importing.", e);
		}
	}
	...
	public void rewriteFile(List<Vehicle> vehicleList) {
		...
		StringBuffer vehicleStringForRevrite = new StringBuffer();
		Iterator<Vehicle> vehicleIterator = vehicleList.iterator();
		while(vehicleIterator.hasNext()) {
			Vehicle vehicle = vehicleIterator.next();
			prepareTheVehicleForRewriting(vehicleStringForRevrite, vehicle);
			vehicleStringForRevrite.append("\r\n");
		}
		
		try {
		Path path = Paths.get(getClass().getResource(filePath).toURI());
			Files.writeString(path, vehicleStringForRevrite.toString());
		} catch (IOException | URISyntaxException e) {
			throw new RuntimeException("An erorr occured while writing the file", e);		}
	}
